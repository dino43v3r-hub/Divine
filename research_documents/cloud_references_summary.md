# Cloud References Summary

This file is generated by internet_source_collector.py so the main analyzer can include reference metadata in pattern analysis.
It stores summaries and citations only, not full copyrighted source text.

## Uncertainty

- Tags: quantum_science_guardrails
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1007/978-3-319-39756-6

No summary available in metadata.

## Uncertainty and probability for branching selves

- Tags: quantum_science_guardrails
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1016/j.shpsb.2006.02.001

No summary available in metadata.

## Uncertainty: The Soul of Modeling, Probability & Statistics

- Tags: quantum_science_guardrails
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://babordplus.u-bordeaux.fr/notice.php?q=id:2788176

OpenAlex abstract metadata available.

## ff19SB: Amino-Acid-Specific Protein Backbone Parameters Trained against Quantum Mechanics Energy Surfaces in Solution

- Tags: quantum_science_guardrails
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1021/acs.jctc.9b00591

OpenAlex abstract metadata available.

## AI, Ethics, and Human Rights

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4018/979-8-3693-9035-1.ch005

The rapid integration of artificial intelligence (AI) into critical domains such as healthcare,
governance, and education has sparked significant concerns about its implications for human rights.
While AI holds the potential to enhance welfare, improve service accessibility, and monitor
compliance with human rights standards, its unregulated use poses risks. Privacy invasion,
algorithmic bias, and erosion of individual autonomy threaten to undermine fundamental human rights,
raising questions about the ethical and legal frameworks governing AI technologies. Addressing this
dual potential of AI has become a critical challenge for policymakers and stakeholders worldwide.
This paper explores th

## AI, Ethics, and Human Rights

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4018/979-8-3693-9035-1.ch001

The rapid development of Artificial Intelligence (AI) has sparked a global conversation about its
ethical implications, particularly its potential to impact human rights. This paper critically
examines the intersection of AI, ethics, and human rights by exploring how AI technologies challenge
the foundations of human dignity, autonomy, and equality. It discusses both the risks and
opportunities posed by AI in relation to human rights, with a focus on key areas such as privacy,
freedom of expression, equality, and access to resources. The paper also offers recommendations for
ethical AI governance that upholds human rights. This paper structure allows for a comprehensive
examination of the et

## Book Review: On Human Dignity

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1177/0040571x8508800521

No summary available in metadata.

## Ethical and Legislative Perspectives on Liver Transplantation in the People's Republic of China†

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1002/lt.21081

OpenAlex abstract metadata available.

## Exploring The Ethical Governance of Artificial Intelligence from An Islamic Ethical Perspective

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.22452/fiqh.vol22no1.5

OpenAlex abstract metadata available.

## Gene-edited babies: Chinese Academy of Medical Sciences’ response and action

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1016/s0140-6736(18)33080-0

No summary available in metadata.

## Human Dignity and the Ethics and Regulation of Technology

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1093/oxfordhb/9780199680832.013.8

This chapter investigates how human dignity might be understood as a normative concept for the
regulation of technologies. First, various distinctions that are relevant for the way human dignity
can be understood are discussed. It is argued that it is particularly important that we should see
human dignity as a concept that ascribes a specific status that forms the basis of the human rights
regimes. Second, the author’s own approach, inspired by Kant and Gewirth, is presented, it being
proposed that we should see the concrete content of human dignity as the protection of the authority
of human beings to govern their own lives. Third, various consequences for the evaluation of
technologies ar

## IFAA recommendations for the ethical use of anatomical images

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1002/ase.2349

OpenAlex abstract metadata available.

## ISLAMIC PERSPECTIVES ON AI AND CYBERSECURITY: DEVELOPING ETHICAL FRAMEWORKS FOR AUTONOMOUS SECURITY SYSTEMS

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.35631/ijlgc.1040041

OpenAlex abstract metadata available.

## Quality of care for pregnant women and newborns—the WHO vision

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1111/1471-0528.13451

OpenAlex abstract metadata available.

## The Age of AI: Artificial Intelligence and the Future of Humanity

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.56315/pscf12-20thacker

OpenAlex abstract metadata available.

## The Dignity of Being and Sexism

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctvpj7904.10

No summary available in metadata.

## The Global Traffic in Human Organs

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1086/300123

OpenAlex abstract metadata available.

## The Good Sense of Dignity: Six Antidotes to Dignity Fatigue in Ethics and Law

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5871/bacad/9780197265642.003.0035

Six issues that seem particularly important for examining human dignity as an ethical and legal
concept are explored. First, the problems of its genealogy and second, the elements of its content
are critically reconstructed. Third, questions of justification are addressed, in particular drawing
attention to the axiological power of secular, egalitarian humanism for the theory of legitimacy of
human dignity. Fourth, problems of concretisation are discussed that concern the precise
circumscription of the scope (and equally importantly) the limits of dignity if it is to be applied
convincingly in concrete legal circumstances. Fifth, the problem of universality is examined.
Finally, some thought

## Ubuntu Ethics, Umuntu, and Human Dignity

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4324/9781003514213-2

No summary available in metadata.

## Ubuntu Ethics, ubuntu and Human Dignity

- Tags: technology_ethics
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4324/9781003514213-3

No summary available in metadata.

## We, the Data

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.7551/mitpress/14703.001.0001

OpenAlex abstract metadata available.

## What is mental health? Evidence towards a new definition from a mixed methods multidisciplinary international survey

- Tags: technology_ethics
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1136/bmjopen-2014-007079

OpenAlex abstract metadata available.

## Beyond “identity”

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1023/a:1007068714468

No summary available in metadata.

## Common good in the theology of John Calvin

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://hdl.handle.net/1842/6433

OpenAlex abstract metadata available.

## Communion Ecclesiology in a Racially Polarised South Africa

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.18820/9781920382971

OpenAlex abstract metadata available.

## Creation and Redemption: Christ, the Wisdom of God. A Study in Divine and Human Action

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9780567690289.ch-008

No summary available in metadata.

## Creation and redemption according to Paul's theology

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1163/9789004266032_004

No summary available in metadata.

## DOGMA TRITUNGGAL MENURUT KRISTEN SAKSI-SAKSI YEHUWA

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.14421/rejusta.2017.1302-01

OpenAlex abstract metadata available.

## Doctrinal Synthesis on the Father, Son, and Holy Spirit

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctt3fgqdw.10

No summary available in metadata.

## Doctrine of the Holy Spirit in John Calvin

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://hdl.handle.net/1842/10289

OpenAlex abstract metadata available.

## Father, Son, and Holy Spirit in the Old Testament

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctt1tm7j2q.6

No summary available in metadata.

## God as one, with reference to Barth and the perichoresis doctrine

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.4102/ve.v44i1.2711

OpenAlex abstract metadata available.

## God the Trinity:

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctt1cgf175.11

No summary available in metadata.

## HERMENEUTICAL BUNGEE-JUMPING: SUBORDINATION IN THE GODHEAD

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://www.etsjets.org/files/JETS-PDFs/40/40-1/40-1-pp057-068_JETS.pdf

OpenAlex abstract metadata available.

## How augustine used the trinity: Functionalism and the development of doctrine

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://ixtheo.de/Record/1645122360

OpenAlex abstract metadata available.

## John Calvin's Soteriology: On the Multifaceted ‘Sum’ of the Gospel

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1111/j.1468-2400.2009.00470.x

OpenAlex abstract metadata available.

## PANDANGAN PENGAKUAN IMAN NICEA-KONSTANTINOPEL (KREDO NICEA) TENTANG DOKTRIN ALLAH TRITUNGGAL

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.46965/jtc.v3i1.249

OpenAlex abstract metadata available.

## Redemption and Creation

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9781978720497.ch-1

No summary available in metadata.

## THE TRINITY IN CHRISTIAN LIFE

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.46362/quaerens.v7i1.301

OpenAlex abstract metadata available.

## The Forgotten Trinity

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9780567690289.ch-001

No summary available in metadata.

## The Hallowing of Logic: The Trinitarian Method of Richard Baxter's Methodus Theologiae

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://hdl.handle.net/1842/7821

OpenAlex abstract metadata available.

## The Holy Spirit who with the Father and the Son together is Worshipped and Glorified

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9780567690289.ch-005

No summary available in metadata.

## The Impact of Calvin’s Trinitarian Controversies upon the Economic-Trinitarian Emphasis of His Theology

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1093/oso/9780195086003.003.0003

Abstract By and large, sufficient attention has not been given in assessments of Calvin’s thought to
the prominence of trinitarian concerns in the doctrinal controversies of his minis-try, or to the
profound contribution that these encounters made to the trinitarian shape of his emerging Christian
vision. As a consequence, Calvin scholarship has tended to underestimate the pervasiveness of his
trinitarian concern.1 The problem is only one facet of a broader inclination in Calvin studies to
divorce consideration of his doctrine from examination of his life and ministry, and vice versa.

## The International Ecumenical Peace Convocation: Towards an Ecumenical Theology of Just Peace?

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1111/j.1758-6623.2010.00092.x

OpenAlex abstract metadata available.

## The Paschal Event:

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctv1hztrfz.20

No summary available in metadata.

## The Revelation of the Father, Son, and Holy Spirit

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/j.ctt3fgqdw.7

No summary available in metadata.

## The Trinity

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1017/9781009293105

OpenAlex abstract metadata available.

## The lord of creation and redemption according to pauline theology

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1163/9789004266032_007

No summary available in metadata.

## Theology's Need for a New Interpretation of Nature: Correlate of the Doctrine of Grace

- Tags: trinity
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1111/j.1540-6385.2007.00334.x

OpenAlex abstract metadata available.

## “In the Name of the Father, Son, and Holy Spirit”: Trinitarian Theology

- Tags: trinity
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9780567677167.ch-008

No summary available in metadata.

## A Way of God's Theodicy: Honesty, Presence, Adventure

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1177/002234097803200404

No summary available in metadata.

## Assessing Existing Theodicies

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9781978720480.ch-2

No summary available in metadata.

## Chaplaincy, Spiritual Care and Moral Injury: Considerations Regarding Screening and Treatment

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.3389/fpsyt.2018.00619

OpenAlex abstract metadata available.

## Climate Grief and Transformation: Deepening Pastoral Eco-Theological Grief Care Through Self-reflection, Witnessing and Narrative Practice

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/10649867.2025.2607257

No summary available in metadata.

## Dangerous prayer: discovering a missional spirituality in the lord’s prayer

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/1756073x.2018.1500163

No summary available in metadata.

## Death and Grief

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4324/9781315808499-21

No summary available in metadata.

## Doing practical theology ‘from the place where it hurts’: the significance of trauma theology in renewing a practical theology of suffering

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/1756073x.2023.2277004

No summary available in metadata.

## Faith after an Earthquake: A Longitudinal Study of Religion and Perceived Health before and after the 2011 Christchurch New Zealand Earthquake

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1371/journal.pone.0049648

OpenAlex abstract metadata available.

## Gendered Political Violence and the Church in Africa: Perspectives from Church Leaders

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.3390/rel16091213

OpenAlex abstract metadata available.

## Going back to my roots : a critical understanding of the interplay between Christian faith and Shona tradition in the quest to find meaning within the HIV pandemic

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://hdl.handle.net/10019.1/6511

OpenAlex abstract metadata available.

## Hamartology and ecology: a critical assessment of Dietrich Bonhoeffer’s view on the nature of sin

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: http://hdl.handle.net/11394/3272

OpenAlex abstract metadata available.

## Hospice spiritual care and suffering: towards a theodicy of suffering

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/13520806.2000.11758892

No summary available in metadata.

## Lament as a Contributor to the Healing of Trauma: an Application of Poetry in the Form of Biblical Lament

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1007/s11089-018-0851-z

No summary available in metadata.

## Mary Shelley's Frankenstein

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.5040/9781682661109

OpenAlex abstract metadata available.

## Moral Injury: An Increasingly Recognized and Widespread Syndrome

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1007/s10943-021-01328-0

No summary available in metadata.

## Nederduitse Gereformeerde Teologiese Tydskrif

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.5952/52-0-39

OpenAlex abstract metadata available.

## Prayer and Practical Theology

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1515/ijpt-2012-0018

Abstract The command to pray invites believers to critically engage with their broken reality with a
view of transforming it in the light of the new reality in which they participate in Christ.
Practical theology, operating in the context of the bi-polar and tense relationship between theory
and praxis, should be expanded to accommodate prayer as the inner mode of its operations to embrace
the existential dimension of the faith praxis, instead of simply limiting itself to a socio-
scientific empirically based descriptive paradigm. This implies that practical theology has to be
embedded within the church as the domain of faith. Prayer, understood within the context of
practical theology, offer

## RECOVERING GRIEF IN THE AGE OF GRIEF RECOVERY

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1179/jpt.2003.13.1.005

No summary available in metadata.

## Shades of Lament: Phenomenology, Theopoetics, and Pastoral Theology

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1007/s11089-014-0616-2

No summary available in metadata.

## Suffering in worship: Anglican liturgy in relation to stories of suffering people

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/1756073x.2018.1459119

No summary available in metadata.

## The Political Unconscious: Narrative as a Socially Symbolic Act

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.2307/3684627

OpenAlex abstract metadata available.

## The Trinity and Suffering

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4324/9781315605364-9

No summary available in metadata.

## The True, the Good and the Reasonable: The Theological and Ethical Roots of Public Reason in Islamic Law

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1017/s084182090000432x

OpenAlex abstract metadata available.

## Theodicy in the Face of Children's Suffering and Death

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.1177/002234099404800207

Discusses the complex issues of Christianity's perspectives and theologies regarding the death of
children and illustrates how such contemplations get expressed in particular pastoral responses.

## Third-Generation Holocaust Representation : Trauma, History, and Memory

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.26530/oapen_628783

OpenAlex abstract metadata available.

## Towards a theology of Covid: providence and lament in past, present, and future trauma narratives

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.1080/1756073x.2020.1861403

OpenAlex abstract metadata available.

## Unanswered Prayer

- Tags: unresolved_suffering
- Provider: Crossref
- Quality: scholarly metadata
- URL: https://doi.org/10.4324/9781315808499-18

No summary available in metadata.

## When faith does violence: Reimagining engagement between churches and LGBTI groups on homophobia in Africa

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.4102/hts.v72i1.3511

OpenAlex abstract metadata available.

## “I Will Show You My Faith by My Works”: Addressing the Nexus between Philosophical Theodicy and Human Suffering and Loss in Contexts of ‘Natural’ Disaster

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.3390/rel10030213

OpenAlex abstract metadata available.

## “The wounded Beast?” - Single Women, Tradition, and the Bible in Zimbabwe

- Tags: unresolved_suffering
- Provider: OpenAlex
- Quality: scholarly metadata
- URL: https://doi.org/10.20378/irb-48992

OpenAlex abstract metadata available.
